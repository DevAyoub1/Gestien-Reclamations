...
export default function UserReclamations () {
    const [listReclamation,setReclamation]=useState([])
    const [pageNumber, setPageNumber]=useState(1)
    const {user}=UserContext()
    const ListReclamation=async(pageNumber)=>{
        await Axios.get(`/api/UserReclamation/${user.id}?page=${pageNumber}`).then((res)=>setReclamation(res.data.data))
    }
  ...
  return (
    ...
    <Box sx={{ width: '90%' }} >
              ...
              <tr key={row.id}>
                ...
                <td>
                  <Box sx={{ display: 'flex', gap: 1 }}>
                   <Link to={`/UserReclamationEdit/${row.id}`}> <Button size="sm" variant="plain" color="neutral">
                      Edit
                    </Button></Link>
      ...
      count={10} shape="rounded"  onChange={(e,value)=>setPageNumber(value)} />
