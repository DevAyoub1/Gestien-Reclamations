...
export default function ListObservation () {
    ...
    const ListObservation=async()=>{
        await Axios.get('/api/observations?page='+currentPage).then((res)=>setObservation(res.data.data))
    }
        ...
        buttons: [
          {
            label: 'Yes',
            onClick: async () => {
              await ReclamationApi.deleteObservation(id);
              ListObservation(currentPage);
            }
          },
          {
            label: 'No',
            ...
            {listObservation.map((row) => (
              <tr key={row.id}>
                ...
                <td>
                  ...
                  <Link to={`/EditObservation/${row.id}`}>  <Button size="sm" variant="plain" color="neutral">
                      <Edit></Edit>
                    </Button></Link>
